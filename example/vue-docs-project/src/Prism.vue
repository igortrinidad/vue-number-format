<template>
  <div class="w-full flex flex-col items-end bg-black relative">
    <button ref="copy" class="absolute top-0 right-0 top-3 right-3 text-center text-white text-xs focus:outline-none z-10">
      <span v-if="!isCopied">copy</span>
      <span v-else class="font-bold text-green">copied ✓</span>
    </button>
    <pre class="w-full" :class="className" v-html="html"></pre>
  </div>
</template>
